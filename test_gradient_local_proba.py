...
for benchmark, protected_attribs in benchmarks_list:
    ...
    eidig_probabilities, maft_probabilities, eidig_time_cost, maft_time_cost = experiments.gradient_comparison_local_probability(benchmark, data, protected_attribs, constraint,
                                                                                                       model, g_num,
                                                                                                       perturbation_size)
    result = [benchmark, eidig_probabilities, maft_probabilities, eidig_time_cost, maft_time_cost]
...
dir = 'logging_data/probabilities_comparison/'
...
# 提取benchmark名称、EIDIG梯度归一化概率、MAFT梯度归一化概率
benchmark_names = results[:, 0]
eidig_probabilities = results[:, 1]
maft_probabilities = results[:, 2]
...
for i in range(len(benchmark_names)):
    ...
    eidig_direction = eidig_probabilities[i]
    maft_direction = maft_probabilities[i]
...
plt.savefig('logging_data/probabilities_comparison/' + iter + 'average_cosine_similarity.png')
plt.show()
...
plt.savefig('logging_data/probabilities_comparison/' + iter + 'cosine_similarity_box.png')
plt.show()
...
plt.ylabel('Cosine Similarity')
plt.title('Cosine Similarity for Each Instance in Each Benchmark')
plt.savefig('logging_data/probabilities_comparison/' + iter + 'cosine_similarity_violin.png')
plt.show()
...
plt.legend()
plt.savefig('logging_data/probabilities_comparison/' + iter + 'time_cost.png')
plt.show()
